...
pub fn crlf(mut i: &[u8], mut state: CRLFState) -> IResult<&[u8], CRLFState> {
    loop {
        match state {
            ...
            _ => { break Ok((i, CRLFState::Complete)); }
        };
    }
}
...
pub fn digit_stream(mut i: &[u8], mut state: DigitState) -> IResult<&[u8], DigitState> {
    match state {
        DigitState::NeedNumbers(mut nums) => {
            match digit0(i) {
                Ok((o, nums_bytes)) => {
...
pub fn take_ignore_stream(mut i: &[u8], mut state: TakeIgnore) -> IResult<&[u8], TakeIgnore> {
    match state {
        TakeIgnore::NeedTake(mut size) => {
            match take(size)(i) {
                Ok((o, _)) => {
                    Ok((o, TakeIgnore::Complete))
                }
                Err(Incomplete(_)) => {
                    Ok((&i[i.len()..], TakeIgnore::NeedTake(size- i.len() as u64)))
                }
                Err(e) => {
                    Err(e)
                }
...
fn test_crlf() {
    ...
    let b = b"\ra";
    assert_eq!(crlf(b, CRLFState::NeedCR).unwrap_err(),
...
fn test_digit() {
    let b = b"111a";
    assert_eq!(digit_stream(b, DigitState::NeedNumbers(vec![])).unwrap().1,
               DigitState::Complete(b"111".to_vec()));
    let b = b"111";
    assert_eq!(digit_stream(b, DigitState::NeedNumbers(vec![])).unwrap(),
...
fn test_take_ignore_stream() {
    let b = b"111";
    assert_eq!(take_ignore_stream(b,TakeIgnore::NeedTake(3)).unwrap(),
               (&b[b.len()..],TakeIgnore::Complete));
    let b = b"111";
    assert_eq!(take_ignore_stream(b,TakeIgnore::NeedTake(100)).unwrap(),
               (&b[b.len()..],TakeIgnore::NeedTake(97)));
    let b = b"1111111";
    let (_,take_ignore) = take_ignore_stream(&b[..3],TakeIgnore::NeedTake(7)).unwrap();
    assert_eq!(take_ignore_stream(&b[3..],take_ignore).unwrap(),
               (&b[b.len()..],TakeIgnore::Complete))
