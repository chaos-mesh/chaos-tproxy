...
impl Proxy {
    ...
    pub async fn exec(&mut self, config : ProxyRawConfig) -> anyhow::Result<()> {
        ...
        tokio::spawn( async move {
            let _ = server.listen(listener).await.map_err(
                |e| tracing::error!("{:?}",e)
            );
        });
        ...
        let exe_path = match std::env::current_exe() {
            Err(e) => {
                return Err(anyhow::anyhow!("failed to get current exe path,error : {:?}", e));
            },
            Ok(path) =>{path}
        };
        ...
        (&mut proxy)
            ...
            .arg(exe_path.clone())
            .arg(format!("-{}",String::from_utf8(vec![b'v'; self.opt.verbose as usize]).unwrap()))
            .arg("--proxy")
            .arg(format!("--ipc-path={}",opt.ipc_path.to_str().unwrap()));

        let rx = self.rx.take().unwrap();
        self.task = Some(tokio::spawn(async move {
            let mut process = match proxy
                .stdin(Stdio::piped())
                .spawn() {
                Ok(process) => {process},
                Err(e) => {
