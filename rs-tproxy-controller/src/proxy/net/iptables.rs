...
pub fn set_iptables_safe(net_env: &NetEnv) -> Vec<Vec<&str>> {
    vec![
        ip_netns(&net_env.netns,vec!["iptables", "-t", "mangle", "-I", "PREROUTING", "-p", "tcp", "--dport", "81:1025", "-s", &net_env.ip,"-j","ACCEPT"]),
        ip_netns(&net_env.netns,vec!["iptables", "-t", "mangle", "-I", "PREROUTING", "-p", "tcp", "--sport", "81:1025","-d", &net_env.ip,"-j","ACCEPT"]),
        ip_netns(&net_env.netns,vec!["iptables", "-t", "mangle", "-I", "PREROUTING", "-p", "tcp", "--dport", "1:80","-s", &net_env.ip, "-j","ACCEPT"]),
        ip_netns(&net_env.netns,vec!["iptables", "-t", "mangle", "-I", "PREROUTING", "-p", "tcp", "--sport", "1:80","-d", &net_env.ip,"-j","ACCEPT"]),
    ]
}